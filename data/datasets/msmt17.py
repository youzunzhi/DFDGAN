...
class MSMT17(BaseImageDataset):
    ...
    def __init__(self,root='/home/haoluo/data', verbose=True, setid=0,
        ...
        train = self._process_dir(self.train_dir, self.list_train_path, base_pid=base_train_pid, base_camid=base_train_camid, setid=setid)
        #val, num_val_pids, num_val_imgs = self._process_dir(self.train_dir, self.list_val_path)
        query = self._process_dir(self.test_dir, self.list_query_path, base_pid=base_query_pid, base_camid=base_query_camid, setid=setid)
        gallery = self._process_dir(self.test_dir, self.list_gallery_path, base_pid=base_gallery_pid, base_camid=base_gallery_camid, setid=setid)
        if verbose:
            print("=> MSMT17 loaded")
    ...
    def _process_dir(self, dir_path, list_path, base_pid=0, base_camid=0, setid=0):
